...
def find_unexplored_path(cur_room, seen, visited):
    ...
    for exit, known in shuffled:
        if known == "?":
            ...
            add_to_seen(room_in_direction, seen, cur_room,
                        exit, get_opposite_direction(exit))
            return exit
...
def find_path(player, seen, visited, path_taken, world):
    ...
    while len(visited) < len(world.rooms):
        ...
        while can_move:
            ...
            if path:
                move_through_path(player.current_room, [
                                  path], visited, player, path_taken)
            else:
                can_move = False

        next_path = nearest_unexplored_room(player.current_room, seen, visited)
        if next_path:
            move_through_path(player.current_room, next_path,
                              visited, player, path_taken)
